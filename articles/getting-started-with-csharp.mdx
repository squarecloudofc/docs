...
- Square Cloud by default uses `restore & run` to start your application. If you want to start in production mode, you'll need to use the `START` field in the configuration file. For more information about the configuration file parameters, visit [configuration parameters](https://docs.squarecloud.app/getting-started/config-file#configuration-parameters).
