...
This error occurs because the `better-sqlite3` library has not been updated to the latest version. In `better-sqlite3` **7.x.x and earlier**, the library **was not support Node.js LTS v20**.x.x.
...
After checking the latest version of the library, you can update it in your `package.json`. For example, if the current version is 9.2.2, you can use the following:
